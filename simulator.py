...
class Tasks(Thread):
    ...
    def run(self):
        ...
        while not self.stop:
            ...
            try:
                file_read = open('bulk-{}.json'.format(self.agent_data['id']), 'r')
            ...
            file_write = open('bulk-{}.json'.format(self.agent_data['id']), 'w')
    ...
    def task_simulator(self):
        ...
        while True:
            ...
            if self.current_task_index != -1:
                try:
                    bulk_read = open('bulk-{}.json'.format(self.agent_id), 'r')
                    self.bulk_data = json.loads(bulk_read.read())
                    if self.bulk_data == {}:
                        self.home_simulator(1)
                    bulk_read.close()
                except:
                ...
                if self.bulk_data is None:
                    ...
                    try:
                        data = data['task_details'][self.current_task_index]
                    except:
                        data = data[self.current_task_index]
                    ...
                    if status != 'Done' and activity_id != '':
                        ...
                        self.bulk_data = edit_task_function(agent_id=self.agent_id, result=self.bulk_data, flag=1,
                                                            activity=activity_id, status=self.current_activity_status,
                                                            data=data, task_id=self.current_task_id)

                    else:

                        self.task_complete.append(self.current_task_id)

                        print 'calling for new task \n'
                        self.home_simulator(0)
                        if self.status:
                            break
                ...
                while True:
                    if (datetime.datetime.now() - start).total_seconds() >= INTERVAL_BULK_UPDATE:
                        break
                self.home_simulator(1)
